...
const mobileNewGameButton = requireElement<HTMLButtonElement>(
...
function bindControls(): void {
  ...
  requireElement<HTMLButtonElement>("reset-view").addEventListener(
    "click",
    () => renderer.resetView(),
  );
  requireElement<HTMLButtonElement>("new-game-button").addEventListener(
    "click",
    showNewGameDialog,
  );
  mobileNewGameButton.addEventListener("click", showNewGameDialog);
  requireElement<HTMLButtonElement>("help-button").addEventListener(
    "click",
    showHelpDialog,
  );
...
function onPointerMove(event: PointerEvent): void {
  ...
  if (pointers.size === 1 && pointer.moved) {
    ...
    if (previousPinchCenter) {
      renderer.panBy(
        center.x - previousPinchCenter.x,
        center.y - previousPinchCenter.y,
      );
    }
...
function handleCell(hex: Hex): void {
  ...
  if (!state.outcome) {
    ...
    if (matching.length > 1) {
      ...
      render();
      announce(
        "Hay varios objetivos en esa casilla. Elige aire o suelo en el panel.",
      );
      return;
    }
  }
  ...
  const pieces = [occupancy.ground, occupancy.air].filter(
...
function render(): void {
  ...
  const focusValue = focusAttribute
    ? activeElement?.getAttribute(focusAttribute)
    : null;
  ...
  const legalActions = selectedId
    ? getLegalActionsForPiece(state, selectedId)
    : [];
...
function firingRangePreview(
  piece: Piece,
): { position?: Hex; cannon?: Direction } | undefined {
  ...
  if (action.kind === "move") {
    return {
      position: action.to,
      cannon:
        piece.type === "medium" ? (action.cannon ?? piece.cannon) : undefined,
    };
  }
  return undefined;
}
...
function renderPieceCard(piece?: Piece): void {
  ...
  const q =
    piece.position.q >= 0 ? `+${piece.position.q}` : `${piece.position.q}`;
  const r =
    piece.position.r >= 0 ? `+${piece.position.r}` : `${piece.position.r}`;
...
function renderActionControls(
  ...
  if (mode.kind === "pieceChoice") {
    actionControls.innerHTML = `<div class="control-section"><h3>Casilla apilada</h3><p>Selecciona capa para inspeccionar.</p><div class="choice-list">${mode.pieceIds
      ...
      .join("")}</div></div>`;
    actionControls
      .querySelectorAll<HTMLButtonElement>("[data-piece-choice]")
      .forEach((button) => {
        button.addEventListener("click", () =>
          selectPiece(button.dataset.pieceChoice ?? ""),
        );
      });
    return;
  }
  ...
  if (mode.kind === "actionChoice") {
    ...
    actionControls
      .querySelectorAll<HTMLButtonElement>("[data-action-choice]")
      .forEach((button) => {
        button.addEventListener("click", () => {
          const action =
    ...
    const selectedDirection =
      pendingAction?.kind === "rotate"
        ...
        : pendingAction?.kind === "orient"
          ...
          : null;
    const title =
      mode.kind === "rotate" ? "Orientar Soldado" : "Orientar cañón";
    actionControls.innerHTML = directionPanel(
      title,
      current,
      selectedDirection,
      "direction-order",
  ...
  const canTransform = legalActions.some(
  ...
  actionControls
    .querySelector('[data-command="rotate"]')
  ...
  actionControls
    .querySelector('[data-command="orient"]')
    ?.addEventListener("click", () => {
      mode = { kind: "orient" };
      pendingAction = null;
      render();
  ...
  actionControls
    .querySelector('[data-command="above"]')
    ?.addEventListener("click", () => {
      if (above) setPending(above);
    });
  actionControls
    .querySelector('[data-command="below"]')
    ?.addEventListener("click", () => {
      if (below) setPending(below);
    });
  actionControls
    .querySelector('[data-command="transform"]')
    ?.addEventListener("click", () => {
...
function renderPendingCard(
  ...
  pendingCard
    .querySelectorAll<HTMLButtonElement>("[data-pending-cannon]")
    .forEach((button) => {
      button.addEventListener("click", () => {
        ...
        const replacement = legalActions.find(
          (action) =>
            ...
            equalHex(
              action.to,
              pendingAction && pendingAction.kind === "move"
                ? pendingAction.to
                : action.to,
            ) &&
...
function directionCompass(
  ...
  const centerDirection =
    highlighted === null ? null : directionNameForView(highlighted);
  const centerLabel =
    selected !== null
      ? "SELECCIONADA"
      : current !== null
...
function directionNameForView(direction: Direction): string {
  const viewDirection = ((direction + (state.activePlayer === 0 ? 3 : 0)) %
    6) as Direction;
...
function renderSoundButton(): void {
  ...
  soundButton.setAttribute(
    "aria-label",
    preferences.sound ? "Silenciar sonido" : "Activar sonido",
  );
...
function renderScreenReaderBoard(): void {
  ...
  for (let r = -5; r <= 5; r += 1) {
    ...
    for (let q = -5; q <= 5; q += 1) {
      ...
      const rangeLabel = firingRange.has(hexKey(hex))
        ? ". Alcance potencial de disparo"
        : "";
    ...
    rows.push(
      `<div role="row" aria-rowindex="${r + 6}">${cells.join("")}</div>`,
    );
  }
  srBoard.innerHTML = rows.join("");
  if (focusedHex)
    canvas.setAttribute("aria-activedescendant", accessibleCellId(focusedHex));
...
function announceCell(hex: Hex): void {
  ...
  const inFiringRange = currentFiringRange().some((cell) =>
    equalHex(cell, hex),
  );
  announce(
    `${
  ...
  dialog
    ...
    .forEach((input) => {
      input.addEventListener("input", () => {
        const key = input.dataset.volume as
          | "masterVolume"
          | "musicVolume"
          | "effectsVolume";
...
function showBlockadeDialog(): void {
  ...
  dialog
    .querySelector("[data-accept-blockade]")
    ?.addEventListener("click", () => {
      dialog.close();
      void acceptBlockade();
    });
...
function blockadePreview(): string {
  ...
  if (fortresses.every((piece) => piece.type === "fortress" && piece.hp === 2))
    return "tablas";
...
function showOutcomeDialog(): void {
  ...
  const winnerClass =
    state.outcome.type === "win"
      ? state.outcome.winner === 0
        ? "blue"
        : "amber"
      : "draw";
  const remainingBlue = state.pieces.filter(
    (piece) => piece.owner === 0,
  ).length;
  const remainingAmber = state.pieces.filter(
    (piece) => piece.owner === 1,
  ).length;
  openDialog(`
...
function loadPreferences(): GamePreferences {
  ...
  try {
    ...
    const loaded = stored
      ? { ...fallback, ...(JSON.parse(stored) as Partial<GamePreferences>) }
      : fallback;
    loaded.masterVolume = clampPreference(
      loaded.masterVolume,
      fallback.masterVolume,
    );
    loaded.musicVolume = clampPreference(
      loaded.musicVolume,
      fallback.musicVolume,
    );
    loaded.effectsVolume = clampPreference(
...
function pieceDescription(piece: Piece): string {
  return {
    soldier:
      "Avanza una casilla en su arco frontal. Puede girar como acción exclusiva.",
    capturer:
    ...
    drone:
      "Vuela hasta tres casillas, sobrevuela suelo y puede compartir hexágono.",
    antiAir:
      "Protege siete hexágonos, intercepta Drones y bloquea fuego a distancia.",
    fortress:
      "Objetivo estratégico inmóvil. Su destrucción termina la partida.",
  }[piece.type];
}
